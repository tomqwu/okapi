...
public class demo
{
    public demo() throws Exception
	{
		 ...
		 * compiles the Java program and generates the exchange format using Piccolo XML parser named output
		 * into the parse directory.
         */

		// DAMON: NOTHING GETS PRINTED HERE UNLESS AN ERROR HAS OCCURED DURING REMOVAL OR
		//        COMPILATION OF CERTAIN FILES IN /JAVOKAPI/PARSE/ DIRECTORY.
        //System.out.println(okapi.exchangeFormat("gen_parser","trec04", "gen_exch"));
        /* Given the name of main parameter file responsible for generating runtime format, converts the already
		 * parsed data and generates the runtime format into the bibfiles directory. In case, the Javokapi package
		 * is unable to handle these functions, they can be manually called by executing the shell command "sh X",
		 * where X is the appropriate function being used.
         */
		//okapi.runtimeFormat("2004gendoc", "gen_exch");
        /* Given the name of the main parameter file responsible for generating index of the runtime format, converts
		 * the already parsed data and generates the runtime format into the bibfiles directory. In case, the Javokapi
		 * package is unable to handle these functions, they can be manually called by executing the shell command
		 * "sh X", where X is the appropriate function being used.
         */
		...
		{  	
			...
			// DAMON: NOTHING GETS PRINTED HERE
			System.out.println(okapi.showRecord("197", count));
			System.out.println(count);
		}
		// DAMON: NOTHING GETS PRINTED HERE
		System.out.println(okapi.chooseDB("2009trec"));
        /* 
		...
		File testFile = new File("2004topics.txt"); 
        ...
        /*
         * Given the structured query file, for every query separate the main query terms
		 * and feedback terms and collect in a set
        */
	...
	/*
	 * Given a query a number, a javokapi object and the set of main query terms and feedback terms.
	 * The original input file is qadhoc05 and the feedback discount is 0.11.
	*/
	private void processQuery(int qryno, javokapi okapi, HashSet mainSet, HashSet feedSet) throws Exception
	{
		...
		//write into result\queryNumber
        BufferedWriter output = new BufferedWriter(new FileWriter(""+qryno));
	...
	*/
	private String processFeedback(javokapi okapi, HashSet feedbackSet)
	{
        ...
        while(itFeedback.hasNext())
		{
        	...
        	setResult = setResult + "s=" + setnum + " w=" + weight + " ";
     	}
     	return setResult;
 	}
	...
	*/
	private String processMain (javokapi okapi, HashSet mainSet)
	{
        ...
        while(itMain.hasNext())
		{
			...
			// VERY CONFUSED AS TO WHY THE TOKNEIZER DELIMETER IS + ?!
            StringTokenizer word = new StringTokenizer(sentence, "+");
            while (word.hasMoreTokens())
			{
	        	String phrase1 = word.nextToken();
                ...
                String stTermList = termlist.toString();
				System.out.println("stTermList >>> " + stTermList);
				// DAMON: THE PROBLEM HERE WITH THE CALL OT SETFIND IS THAT WE ARE
				//        PASSING A STRING OF FORM S=1, S=2, AND SO ON. THIS IS
				//        COMPLETELY WRONG, BECAUSE WE ONLY HAVE TO PASS THE SET NUMBER.
				//        IF THE RESULT OF MULTIPLE SETS ARE TO BE RETURNED, MULTIPLE
				//        CALLS TO THIS FUNCTION MUST BE INVOKED.
            	String result = okapi.setFind("1", "","adj");
        	...
        	String stPhraseList = phraselist.toString();
			// DAMON: THE PROBLEM HERE HAS BEEN REPEATED AGAIN. WE ARE
			//        PASSING A STRING OF PHRASES INSTEAD OF INDIVIDUAL NUMBERS.
        	String result = okapi.setFind("8","", "or");
System.out.println("result >>> " + result); 
System.out.println("stPhraseList >>> " + stPhraseList); 
        	String[] parsed = result.split(" np=");
        	String[] parsed2 = parsed[0].split("S");
        	String setnum = parsed2[1];
        	String totalnp = parsed[1].trim();
